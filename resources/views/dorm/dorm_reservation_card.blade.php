...
<script>
    $(document).ready(function() {
        ...
        const reservation_start_dateInput = document.getElementById('reservation_start_date');
        const reservation_start_dateInputFemale = document.getElementById('reservation_start_date_female');
        ...
        $('#availabilityDate').change(function() {
            ...
            const selectedDateDP = $(this);
            const gender = 'male';
            reservation_start_dateInput.value = selectedDate;
        ...
        $('#availabilityDateFemale').change(function() {
            ...
            const selectedDateDP = $(this);
            const gender = 'female';
            reservation_start_dateInputFemale.value = selectedDate;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    inputFemale.max =
                        availabilityFemale; // Update max attribute dynamically
                    // Reset input value if exceeds new max
                    if (parseInt(inputFemale.value) > availabilityFemale) {
        ...
        function updateCheckOutManually(dateInput) {
            var selectedDate = dateInput.datepicker("getDate");
            if (selectedDate !== null) {
                // Add one day to selected date
                var nextDay = new Date(selectedDate);
                nextDay.setDate(selectedDate.getDate() + 1);

                // Find the closest card-body from the dateInput context
                dateInput.closest('.card-body').find(
                        'input[name="reservation_end_date"], input[name="reservation_end_date_female"]')
    ...
    $(document).ready(function() {
        ...
        const datePicker1 = $(
            "#availabilityDate, #reservation_start_date, #reservation_end_date, #availabilityDateFemale, #reservation_start_date_female, #reservation_end_date_female"
        );
        ...
        // Update end date picker to disable selected start date Male
        $("#reservation_start_date").on("change", function() {
            var selectedDate = $(this).datepicker("getDate");
            const selectedDateDefault = $(this).val();
            if (selectedDate !== null) {
                availabilityDateInputMale.value = selectedDateDefault;
                updateAvailableDatePickerManuallyMale(selectedDateDefault);
                // Add one day to selected date
                var nextDay = new Date(selectedDate);
                nextDay.setDate(selectedDate.getDate() + 1);
        ...
        // Update end date picker to disable selected start date Male
        $("#reservation_start_date_female").on("change", function() {
            var selectedDate = $(this).datepicker("getDate");
            const selectedDateDefault = $(this).val();
            if (selectedDate !== null) {
                availabilityDateInputFemale.value = selectedDateDefault;
                updateAvailableDatePickerManuallyFemale(selectedDateDefault);
                // Add one day to selected date
                var nextDay = new Date(selectedDate);
                nextDay.setDate(selectedDate.getDate() + 1);
                $(this).closest('.card-body').find(
